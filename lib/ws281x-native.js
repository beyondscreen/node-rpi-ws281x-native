...
// for GPIO-numbers see: https://pinout.xyz/
...
function init(params) {
  ...
  channelsConfig.forEach((channelParams, channelId) => {
    ...
    // FIXME: use seperate send and prepare-buffers, client side should always see 4byte per LED

    channels[channelId] = {
      channelId,
      params,
      // FIXME: 4 bytes for rgbw LEDs
      buffer: params.count > 0 ? new Buffer(params.count * 3) : null
    };
  });
...
function render() {
  channels.forEach(channel => {
    ...
    bindings.setChannelData(channel.channelId, channel.buffer);
  });
...
function setNamedParams(channelId, params) {
  console.log('setNamedParams', channelId, params);
...
function setNamedParam(channelId, paramName, value) {
  console.log('setNamedParam', channelId, paramName, paramCodes[paramName], value);
