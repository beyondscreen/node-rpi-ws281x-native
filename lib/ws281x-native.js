...
const _update = new Symbol('_update');
const _init = new Symbol('_init');
...
class Channel {
  ...
  [_init]() {
    Object.keys(this.params).forEach(paramName => {
      ...
      bindings.setChannelParam(this.channelId, code, value);
    });
  }
  ...
  [_update]() {
    ...
    bindings.setChannelParam(
      this.channelId,
      paramCodes.brightness,
      this.brightness
    );
    bindings.setChannelData(this.channelId, this.buffer);
  }
}
